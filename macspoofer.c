...
bool chmac(const int8 *If, Mac mac) {
  ...
  printf("Old MAC address : %02x:%02x:%02x:%02x:%02x:%02x\n",
         ir.ifr_hwaddr.sa_data[0], ir.ifr_hwaddr.sa_data[1],
         ir.ifr_hwaddr.sa_data[2], ir.ifr_hwaddr.sa_data[3],
         ir.ifr_hwaddr.sa_data[4], ir.ifr_hwaddr.sa_data[5]);

  memcpy(revert, ir.ifr_hwaddr.sa_data, 6);
